...
def _sync_stock(db, product_id, variant_id, warehouse_id):
    ...
    """, (product_id, variant_id, warehouse_id)).fetchone()[0]
...
def create_request(product_id, variant_id, from_wh, to_wh, qty):
    ...
    FROM stock_batches
    WHERE product_id=? AND variant_id=? AND warehouse_id=?
    """,(product_id, variant_id, from_wh)).fetchone()[0]
...
def approve_request(request_id):
    ...
    try:
        ...
        """,(product_id, variant_id, from_wh)).fetchall()
        ...
        for b in batches:
            ...
            """,(product_id, variant_id, from_wh, b["id"], take))
        ...
        for wh in [from_wh, to_wh]:
            _sync_stock(db, product_id, variant_id, wh)
